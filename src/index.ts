...
function prepararAmbiente() {
  ...
  if (!fs.existsSync(pastaRecibos)) fs.mkdirSync(pastaRecibos);
...
// Funções de Cliente
// ===============================
function cadastrarCliente(nome: string, telefone: string, email: string) {
...
function listarClientes() {
  ...
  linhas.slice(1).forEach((linha: string) => {
...
// ===============================
async function editarCliente(idCliente: number): Promise<void> {
  const conteudo = fs.readFileSync(arquivoClientes, "utf-8");
  const linhas = conteudo.split(/\r?\n/).filter(l => l.trim() !== "");
  if (linhas.length <= 1) {
    console.log("Nenhum cliente cadastrado.");
...
// ===============================
async function excluirCliente(idCliente: number): Promise<void> {
  const conteudo = fs.readFileSync(arquivoClientes, "utf-8");
  const linhas = conteudo.split(/\r?\n/).filter(l => l.trim() !== "");
  if (linhas.length <= 1) {
    console.log("Nenhum cliente cadastrado.");
...
// Funções de Produto
// ===============================
function cadastrarProduto(nome: string, categoria: string, preco: number) {
...
// Funções de Pedido
// ===============================
function registrarPedido(idCliente: number, idsProdutos: number[]) {
  const clientes = fs.readFileSync(arquivoClientes, "utf-8").split(/\r?\n/).filter(l => l.trim() !== "");
...
// ===============================
function appendVendaAoRelatorio(idPedido: string, cliente: string, produtosRaw: string, total: string, pagamento: string, dataISO: string) {
  // nomes de arquivo
...
function removerPedidoPorId(idPedido: string) {
  const conteudo = fs.readFileSync(arquivoPedidos, "utf-8");
  const linhas = conteudo.split(/\r?\n/).filter(l => l.trim() !== "");
...
// ===============================
async function confirmarPedidoFluxo(idPedido: number): Promise<void> {
  const conteudo = fs.readFileSync(arquivoPedidos, "utf-8");
  const linhas = conteudo.split(/\r?\n/).filter(l => l.trim() !== "");
  if (linhas.length <= 1) {
    console.log("Nenhum pedido registrado.");
